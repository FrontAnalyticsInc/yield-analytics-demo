...
7. **Designed experiment (Experiments, 4 min).** *New experiment* → keep commissure height and the two example factors. The noise comes from the line, and the calculator sizes the experiment (≈19 units) and draws the square. Enter an expected change of 0.05 mm for tension to show the "is the gap big enough?" warning. Save → **Simulate results** → Analyze: verdicts, best settings, charts, then *Reveal the simulated truth*.
8. **Scale story (docs/architecture.md, 2 min).** The same schema and app run on Azure: Event Hubs, a lakehouse, Azure ML scoring, Container Apps with Entra SSO, and Power BI with RLS, at roughly $1–2k/month for 20–50 users.
